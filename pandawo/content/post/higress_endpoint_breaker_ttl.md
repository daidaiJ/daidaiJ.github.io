...
------
> 上游换成私有化部署的 LLM 端点之后，故障形态变了：不报 429、不吐 5xx，就是慢——请求挂在网关上几分钟才吐结果，用户早走了。这篇记录我基于 ai-load-balancer 做的一层端点级断路器：响应耗时突破阈值就把端点拉黑，TTL 到期自动放回负载均衡池。整套设计里我最满意的是统计层的 5s 去重窗口——同一端点短时间内只计一次，既防高频请求重复统计，也防一次抖动被放大成误判。
...
所以统计层加了去重窗口：**短时间内（5s）同一端点只计算一次**。设计推演如下（伪代码，非最终实现）：

```go
// 伪代码：响应统计 + 去重 + 拉黑判定
const (
    dedupWindowMs = 5000   // 5s 去重窗口
    slowThreshold = 70000  // 慢阈值 70s
    blockTtlMs    = ttl    // 拉黑 TTL，做成配置项，不写死
)
...
func onUpstreamResponse(ep string, costMs int64) {
    ...
    if now-st.lastCountedAt < dedupWindowMs {
        return                 // ← 窗口内已计过，这个响应不重复入账
    }
    st.lastCountedAt = now
    if costMs >= slowThreshold {
        block(ep, now+blockTtlMs)   // 突破阈值，拉黑
    }
}
```

去重带来的第二个收益是防抖动放大误判。没有窗口时，一次网络抖动产生的整批慢响应会被当成多次独立证据，叠加起来很容易越过判定边界；有窗口后，统计的时间分辨率被限制在 5s，抖动只能贡献一个样本，持续性的慢才会持续入账。
...
## 阈值与 TTL：70s 拉黑，到期自动恢复
------
判定逻辑压成一句话：端点响应耗时突破 70s，且通过了 5s 去重确认，就拉黑；拉黑不是永久的，TTL 到期自动恢复参与负载均衡。

70s 这个数的来源是私有化场景的两头挤压：下限要高于正常推理的长尾——长上下文请求跑三四十秒是常态，阈值太低会把慢请求误判成慢节点；上限受交互式体验约束——LLM 场景用户对首字延迟的容忍就是几十秒的量级，一个响应要 70s 的端点，对用户来说和挂了没有区别。既然"慢到 70s"本身已经是不可接受的服务质量，单次确认即拉黑，不再堆失败次数。

时间线长这样：

```text
T+0s     请求 A → 端点 E，开始推理
T+70s    A 耗时突破 70s → E 记一次慢失败（窗口内首次，入账）
T+71s+   在途请求 B/C/D 相继超时 → 5s 窗口内，全部去重，不再计数
         → E 拉黑，blockUntil = now + TTL
...
> TTL 恢复是个乐观假设：到期不等于健康。端点如果还在慢，第一个打过去的请求会用 70s 重新确认一次，再拉黑一轮——相当于用真实流量做探测，代价是每个 TTL 周期有一次糟糕的用户体验。要不要配主动健康检查（用最小模型探活提前解封），我还在权衡，至少当前流量规模下不值当。
