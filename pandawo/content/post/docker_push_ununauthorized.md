...
> 问题背景:  
> 在通过docker client 库向harbor 仓库推送一个28G 左右的镜像时，会遇到推送到100% 左右出现`unauthorized: unauthorized to access repository: project_name/repo_name `的报错，导致整个job 失败，然后出现重试，导致业务层面看到push 进度从100%--> 0% 的奇怪现象
...
[harbor push 30min超时，出现unauthorized: unauthorized to access repository](https://github.com/goharbor/harbor/issues/19413)    
